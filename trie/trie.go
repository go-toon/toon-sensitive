...
func (t *Trie) Query(text string) (bool, []string, string) {
	...
	for i = 0; i < l; i++ {
		...
		for j = i + 1; j < l; j++ {
			...
			if node.End {
				jj = j //还有子节点到情况， 记住上次找到的位置， 以匹配最大串 (eg: AV, AV女优)
...
// 取前5个字，到前缀白名单中检查
func (t *Trie) isInWhitePrefixList(found []string, chars []rune, i, j, length int) (inWhiteList bool) {
	if i == 0 { // 第一个
		return
	}
